...
<template>
  <div :class="baseClass" >
    <div class="bui-table-wrapper">
      <div
        ref="headerWrapper"
        :style="headerStyles.headerWrapper"
        :class="{
          'bui-table-overflow-y': scrollY,
        }"
        class="bui-table-header"
        ...
        <table-head
          ...
          :header-height="isTableBodyDiv ? turbo.headerHeight : ''"
        />
      </div>
      <div
        v-if="touchCeiling"
      ...
      <div
        ...
        ref="bodyWrapper"
        :class="{
          'bui-table-overflow-x': scrollX,
          'bui-table-overflow-y': scrollY,
          'bui-table-scrolling-left': scrollPosition === 'left',
          'bui-table-scrolling-right': scrollPosition === 'right',
          'bui-table-scrolling-middle': scrollPosition === 'middle',
        }"
        :style="bodyWrapperStyles"
        class="bui-table-body"
      ...
      <div
        v-if="isLeftFixed "
        :style="fixedLeftTableStyles"
        :class="{
          'bui-table-fixed-left': true,
          'bui-table-fixed-left__ceiling': touchCeiling,
        }"
        @mousewheel="handleFixedMouseWheel"
        @DOMMouseScroll="handleFixedMouseWheel"
      >
        <div
          ref="leftHeaderWrapper"
          :style="headerStyles.leftHeaderWrapper"
          class="bui-table-fixed-header">
          <table-head
            :columns="leftColumns"
            :data="rebuildData"
            :column-rows="leftFixedColumnRows"
            :styles="tableStyles"
            :columns-width="columnsWidth"
            :header-height="isTableBodyDiv ? turbo.headerHeight : ''"
            fixed="left"/>
        ...
        <div
          v-if=" data && data.length"
          ref="leftBodyWrapper"
          :style="fixedBodyWrapperStyles"
          class="bui-table-fixed-body"
        >
          <table-body-div
            v-if="isTableBodyDiv"
            :columns="leftColumns"
      ...
      <div
        v-if="isRightFixed "
        :style="fixedRightTableStyles"
        :class="{
          'bui-table-fixed-right': true,
          'bui-table-fixed-right__ceiling': touchCeiling,
        }"
        @mousewheel="handleFixedMouseWheel"
        @DOMMouseScroll="handleFixedMouseWheel">
        <div
          ref="rightHeaderWrapper"
          :style="headerStyles.rightHeaderWrapper"
          class="bui-table-fixed-header">
          <table-head
      ...
      <!-- <div
        ...
        class="bui-table-fixed-right-header"/> -->

    </div>
    <div
      v-show="showResizeLine"
      ref="resizeLine"
      class="bui-table-resizeline"/>
  </div>
</template>

<script>
import {CSS_PREFIX, LOCALE_PREFIX} from '../base.js';
import {isOneOf} from '../utils/helper.js';
...
import locale from '../minxins/locale';
import {
  on,
  off,
  getScrollBarWidth,
...
export default {
  name: `${CSS_PREFIX}table`,
  components: {tableHead, tableBody, tableBodyDiv},
  props: {
    data: {
      type: Array,
      default() {
  ...
  computed: {
    ...
    baseClass() {
      const classArr = [`${CSS_PREFIX}table`];
      if (!this.border) {
        classArr.push(`${CSS_PREFIX}table-no-border`);
      } else if (this.border === 'cellspacing') {
        classArr.push(`${CSS_PREFIX}table-cellspacing-border`);
      }
      return classArr;
    },
    isLeftFixed() {
  ...
  methods: {
    ...
    handleExpandClick(row) {
      ...
      row.isExpand = status;
      this.rebuildData.splice(row._index, 1, row);
      this.$emit('on-expand', {row, index, status});
    },
    // 滚轮事件
